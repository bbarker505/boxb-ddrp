...
# Last modified by B. Barker on 7/29/22
# - 7/29: Fixed sublist object in order to sample correct days in Daily Loop
# - 9/21: Added hack for using climate normal data (PRISM and EOBS) - ln 1331
...
# Create a list of day counts to use for daily loop
# The list starts at 1 even if --start_doy > 1
sublist <- 1:length(start_doy:end_doy)
...
for (i in seq_along(vars)) {
  ...
  if (forecast_data %in% c("PRISM", "PRISM_800m")) {
    ...
    fl_pat <- glob2rx(paste0("*EOBS_", vars[i], "*", start_year, "*.tif$*"))
  } else if (forecast_data == "ClimateEU") {
...
# Stop program if there are any climate data files missing
